...
class Reimporter(object):
    ...
    def _update_object(self, old_object, new_object):
        ...
        for prp in old_object._rel_props:
            ...
            # Special case #6: Group.users
            elif _is_rel(prp, Group.users):
                pass
    ...
    def do_reimport(self):
        ...
        with SessionGen() as session:
            ...
            old_groups = dict((x.name, x) for x in old_contest.groups)
            ...
            new_groups = {g.name: g for g in new_contest.groups}
            ...
            # Do the actual merge: compare all groups of the old and of
            # the new contest and see if we need to create, update or
            # delete them. Delete only if authorized, fail otherwise.
            groups = set(old_groups.keys()) | set(new_groups.keys())
            cur_groups = {}
            for name in groups:
                old_group = old_groups.get(name, None)
                new_group = new_groups.get(name, None)

                if old_group is None:
                    # Create a new group.
                    logger.info("Creating group %s" % name)
                    old_contest.groups.append(new_group)
                    cur_groups[name] = new_group
                elif new_group is not None:
                    # Update an existing group.
                    logger.info("Updating group %s" % name)
                    self._update_object(old_group, new_group)
                    cur_groups[name] = old_group
            ...
            for username in users:
                ...
                if old_user is None:
                    ...
                    new_user.group = cur_groups[new_user.group.name]
                    ...
                    new_user = self.loader.get_user(username)
                    group_name = new_user.group.name
                    new_user.group = None
                    self._update_object(old_user, new_user)
                    old_user.group = cur_groups[group_name]
                else:
            ...
            for name in groups:
                ...
                if old_group is not None and new_group is None:
                    ...
                    if self.force:
                        ...
                        old_contest.groups.remove(old_group)
                    else:
                        logger.critical(
                            "Group %s exists in old contest, but "
                            "not in the new one. Use -f to force." %
                            name)
                        return False
