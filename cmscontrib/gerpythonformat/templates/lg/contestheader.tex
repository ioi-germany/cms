...
\def\logo{
\begin{tikzpicture}
    \clip (0,0) rectangle (7.5cm, 7.2cm);
    \node[anchor=south west] (pic) at (0, 0) {\includegraphics[clip,scale=.55,draft=false]{logo.eps}};

    \begin{scope}[yshift=.75cm]
        \node[white!40!black, rotate=16.125, anchor=west, align=left] at (15:2.25cm)
        {\mbox{}\\[.6ex]{\bfseries\contestname}\\Aufgabe:\enspace\bfseries\taskname\\[-.8ex]};
    \end{scope}
\end{tikzpicture}
}

%\rhead{\vbox to \headh{\vbox to 0pt{\vskip-2in\hbox to 0pt{\hskip\hsize\hskip-1.5in\includegraphics{\barpdf}}}\quad\null}}
\rhead{\vbox to \headh{\vbox to 0pt{\vskip-2in\hbox to 0pt{\hskip\hsize\hskip-1.5in\logo}}\quad\null}}
