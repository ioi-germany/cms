...
Once you start ``cmsGerTranslateWebServer``, you can access the web interface, by default at `<localhost:8892>`_. When someone uploads a translation, it gets stored in the appropriate file in the repository and, if that's actually a git repository, is committed and pushed (if allowed in ``cms.toml``).
